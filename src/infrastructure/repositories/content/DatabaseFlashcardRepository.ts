...
/**
 * Database implementation of Flashcard repository
 */
export class DatabaseFlashcardRepository implements IFlashcardRepository {
  ...
  private mapToFlashcard(row: FlashcardRow): Flashcard {
    return Flashcard.reconstitute(
      ...
      row.question,
      row.answer,
      (row.difficulty || "medium") as DifficultyLevelType,
      (row.computedDifficulty || null) as DifficultyLevelType | null,
      row.createdAt,
    );
  }
}
