...
- Validate form handling uses react-hook-form with shadcn Form components

### Form Handling (Required)
**Always use react-hook-form + shadcn Form + Zod:**
...
- Define Zod schemas in `src/lib/validation.ts` or domain-specific validation files
- Disable submit button until form is valid: `disabled={!form.formState.isValid || isLoading}`
...
- [ ] Form schemas are in `src/lib/validation.ts` or domain-specific files
