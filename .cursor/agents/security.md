...
- **Forms must validate on both client and server** - never trust client-side validation alone
- Verify password requirements are strong (min 8 chars, uppercase, lowercase, number, special char)
- Check that form validation schemas match between client and server

### Injection Prevention
- Check for SQL injection vulnerabilities in database queries
- Verify proper parameterization of queries (better-sqlite3)
...
### Form Security
- **Client-side validation**: Use react-hook-form with Zod for immediate feedback
- **Server-side validation**: ALWAYS validate again in API routes - client validation can be bypassed
- **Password security**: Enforce strong passwords (min 8 chars, uppercase, lowercase, number, special char)
- **Rate limiting**: Implement rate limiting on form submission endpoints (especially auth forms)
- **CSRF protection**: Verify state-changing form submissions have CSRF protection
- **Input sanitization**: Sanitize user input before database operations
- **Error messages**: Don't leak sensitive information in validation errors
- **Form data**: Never log sensitive form data (passwords, tokens, etc.)
...
- [ ] Forms validate on both client (react-hook-form + Zod) and server (Zod)
- [ ] Password validation enforces strong requirements
- [ ] Form submission endpoints have rate limiting
